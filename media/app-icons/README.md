...
The NutriTrack mark: a **brand-orange plate with one quadrant (top-right) cleanly cut out**, three horizontal data lines on the cream area (representing parsed meal data), and one solid orange dot (representing "logged"). See `source/icon-master.svg` for the master vector.

Design rationale: the wedge-cut is the key uniqueness. Most nutrition apps use a fork-on-plate or apple; this reads as a plate with one bite taken out — the system understands what was removed.

Per-platform coverage generated by `scripts/export_app_icons.py`:

- iOS: 9 sizes in `ios/AppIcon.appiconset/` (40, 58, 60, 60, 80, 87, 120, 120, 180, 1024) + `Contents.json`
- Android legacy: 5 density buckets in `android/{m,h,x,xx,xxx}hdpi/ic_launcher.png`
- Android adaptive: 3 layers at 432×432 (`foreground.png`, `background.png`, `monochrome.png`) + mipmap-anydpi-v26 XML
- Web: 4 favicon sizes + multi-size `favicon.ico`
- Play Store: `store-assets/google-play/icon-512.png` (opaque, 512×512)
- Branding exports: 4 PNG sizes in `branding/exports/logo-icon-*.png` (canonical reference exports)

To regenerate everything after editing `icon-master.svg`:

```bash
python3 scripts/export_app_icons.py
```

Visual overview of how the icon scales from 1024→16px: see `docs/icon-sizes-overview.png`.
...
- Web: 4 favicon sizes + multi-size `favicon.ico`
- Play Store: `store-assets/google-play/icon-512.png` (opaque, 512×512)
- Branding exports: 4 PNG sizes in `branding/exports/logo-icon-*.png`
